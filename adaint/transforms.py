...
class RandomCrop(object):
    ...
    def __init__(self, keys, crop_size, isotropic=False):
    ...
    def _get_cropbox(self, img):
        ...
        crop_size = min(self.crop_size, img.shape[0], img.shape[1])

        # if crop_size != 224:
        #     print(self.crop_size, img.shape[0], img.shape[1])

        # Determine the margins for cropping
        margin_h = img.shape[0] - crop_size
        margin_w = img.shape[1] - crop_size

        # Randomly choose an offset within the allowable margin
        offset_h = random.randint(0, margin_h)
        offset_w = random.randint(0, margin_w)

        # Calculate the coordinates of the crop
        crop_y1, crop_y2 = offset_h, offset_h + crop_size
        crop_x1, crop_x2 = offset_w, offset_w + crop_size
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            results[key] = results[key][y1:y2, x1:x2, :]
            results[f'{key}_crop_size'] = results[key].shape[:2]
            # if min(results[key].shape[:2]) != 224:
            #     print(y1, y2, x1, x2, results[key].shape)
        return results
