...
class CLIPIQAFixed(nn.Module):
    def __init__(self, classnames, backbone_name='RN50', pos_embedding=False, normalization="softmax"):
        super().__init__()
    ...
    def forward(self, image):
        ...
        for i in range(self.num_clip):
            ...
            elif self.normalization == "l1norm":
                logits_per_image_ = torch.clip(logits_per_image, 0)
                probs = logits_per_image_ / (logits_per_image_.sum(dim=-1, keepdims=True))
            else:
