...
@PIPELINES.register_module()
class Resize2(Resize):

    def _resize(self, img):
        if self.keep_ratio:
            scale = (int(img.shape[0] / min(img.shape[:2]) * self.scale[0]), int(img.shape[1] / min(img.shape[:2]) * self.scale[0]))
            scale = (max(scale[1], self.scale[0]), max(scale[0], self.scale[0]))
            img, w_scale, h_scale = mmcv.imresize(
                img,
                scale,
                return_scale=True,
...
class ResizePad(Resize):
    ...
    def _resize(self, img):
        ...
        # 差を2で割って、パディングするサイズを計算します。
        # ここで、奇数の場合、片方が1ピクセル多くなるように調整します。
        pad_before = delta // 2
        pad_after = delta - pad_before

        # 幅と高さのどちらが大きいかによってパディングする軸を変えます。
        if width > height:
            # 幅の方が大きい場合、高さにパディングを追加します。
            img = np.pad(img, ((pad_before, pad_after), (0, 0), (0, 0)), mode='reflect')
        else:
            # 高さの方が大きい場合、幅にパディングを追加します。
            img = np.pad(img, ((0, 0), (pad_before, pad_after), (0, 0)), mode='reflect')
