...
def parse_args():
    ...
    parser.add_argument('--four_k', action='store_true')
...
def main():
    ...
    if args.four_k:
        cfg.data.test["dir_lq"] = "data/FiveK/input/JPG/original_rename/"
        cfg.data.test["dir_gt"] = "data/FiveK/expertC/JPG/original_rename/"
        # cfg.data.test["ann_file"] = "data/FiveK/test_original.txt"

    if args.visualize:
        cfg.test_cfg["visualize"] = True
    else:
        cfg.test_cfg["visualize"] = False
    ...
    if rank == 0 and 'eval_result' in outputs[0]:
        ...
        print('Eval-time: {}'.format(np.array(model.module.inference_times[1:]).mean()))
